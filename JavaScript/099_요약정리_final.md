...
## 수업 환경 설정
* about:blank // 다른 브라우저 환경에서는 내부 변수가 있으므로 about:blank에서
...
* 변수 이름 규칙
    ...
    * 대소문자를 구분한다.
    * class와 생성자는 첫 문자를 대문자로, 나머지는 보통 소문자로 시작한다.
* var, let, const
    ```js
    if (true) {
...
* 논리 연산자(&&, ||, !, !!, &, |)
    ...
    * 암기코드
    ...
    ```
    ```js
    // 앞에 값이 널이냐를 확인하고 싶은 경우, 단락 회로 평가라고 부릅니다.(중요)
    result1 = 10 || 100;
    result2 = 0 && 100;
    result3 = null || 100;
    result4 = null && 100;
...
- Number(숫자)
    ...
    * Math
        ...
        * Math.floor()
        * Math.round()
        * Math.random() // 이것도 많이 씁니다.
        * Math.abs()
        * Math.sqrt() // **0.5 강사님은 이렇게 많이 사용합니다.
        * Math.pow()
    * NaN
    * Infinity, -Infinity
...
- String(문자열)
    ...
    * 메서드 : 
        ...
        * str.includes()
        * str.slice()
        * str.splice() // 배열에 있는 메서드입니다. str에는 없습니다.
        * str.split()
        * str.substring()
        * str.substr()
        * str.toLowerCase()
...
- Array(배열) : object
    ...
    * 메서드 : 
    ...
    Array(100).fill(0).map((value, index)=> value + index) // 이 줄이 가장 직관적
